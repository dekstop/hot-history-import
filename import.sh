...
time easy ${DB_CMD} -c "\copy changeset_meta FROM '${ETL_DIR}/changeset_meta.tsv' NULL AS '' csv delimiter '	' header" || exit 1
time easy ${DB_CMD} -c "\copy changeset_meta_tags FROM '${ETL_DIR}/changeset_meta_tags.tsv' NULL AS '' csv delimiter '	' header" || exit 1
...
order by num_users desc;" || exit 1
